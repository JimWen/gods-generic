...
func TestQueuePeek(t *testing.T) {
	queue := New[int](3)
	var empty int
	if actualValue, ok := queue.Peek(); actualValue != empty || ok {
...
func TestQueueSerialization(t *testing.T) {
	...
	var err error
	assert := func() {
		if actualValue, expectedValue := fmt.Sprintf("%s%s%s", utils.GenericToInterfaceSlice(queue.Values())...), "abc"; actualValue != expectedValue {
			t.Errorf("Got %v expected %v", actualValue, expectedValue)
		}
